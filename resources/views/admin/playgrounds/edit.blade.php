...
@section('title', 'Edit Playground')
@section('page-title', 'Edit Playground')
@section('page-subtitle', 'Update playground information')

@section('content')
    <div class="max-w-4xl mx-auto">
        <div class="bg-white rounded-2xl shadow-sm border border-gray-50 p-8">
            <form action="{{ route('admin.playgrounds.update', $playground) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-2 gap-6">
                    <div class="mb-4">
                        ...
                        <select name="owner_id" id="owner_id"
                            ...
                            @foreach($owners as $owner)
                                <option value="{{ $owner->id }}" {{ old('owner_id', $playground->owner_id) == $owner->id ? 'selected' : '' }}>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $playground->name) }}"
                    ...
                    <div class="mb-4">
                        ...
                        <select name="city_id" id="city_id"
                            ...
                            @foreach($cities as $city)
                                <option value="{{ $city->id }}" {{ old('city_id', $playground->city_id) == $city->id ? 'selected' : '' }}>
                                    {{ $city->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-4">
                        ...
                        <select name="zone_id" id="zone_id"
                            ...
                            @foreach($zones as $zone)
                                <option value="{{ $zone->id }}" {{ old('zone_id', $playground->zone_id) == $zone->id ? 'selected' : '' }}>
                ...
                <div class="grid grid-cols-3 gap-6">
                    <div class="mb-4">
                        ...
                        <select name="type" id="type"
                            ...
                            <option value="football" {{ old('type', $playground->type) == 'football' ? 'selected' : '' }}>
                                Football</option>
                            <option value="basketball" {{ old('type', $playground->type) == 'basketball' ? 'selected' : '' }}>
                                Basketball</option>
                            <option value="tennis" {{ old('type', $playground->type) == 'tennis' ? 'selected' : '' }}>Tennis
                            </option>
                            <option value="volleyball" {{ old('type', $playground->type) == 'volleyball' ? 'selected' : '' }}>
                                Volleyball</option>
                            <option value="multi" {{ old('type', $playground->type) == 'multi' ? 'selected' : '' }}>Multi
                            </option>
                        </select>
                        @error('type')
                            <p class="text-red-500 text-sm mt-1.5">{{ $message }}</p>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="number" name="price_per_hour" id="price_per_hour"
                            value="{{ old('price_per_hour', $playground->price_per_hour) }}" step="0.01" placeholder="0.00"
                    ...
                    <div class="mb-4">
                        ...
                        <input type="url" name="location_url" id="location_url"
                            value="{{ old('location_url', $playground->location_url) }}"
                ...
                <div class="grid grid-cols-2 gap-6">
                    <div class="mb-4">
                        ...
                        <input type="number" name="min_booking_hours" id="min_booking_hours"
                            value="{{ old('min_booking_hours', $playground->min_booking_hours) }}"
                    ...
                    <div class="mb-4">
                        ...
                        <input type="number" name="max_booking_hours" id="max_booking_hours"
                            value="{{ old('max_booking_hours', $playground->max_booking_hours) }}"
                ...
                <div class="grid grid-cols-2 gap-6">
                    <div class="mb-4">
                        ...
                        <input type="time" name="opening_time" id="opening_time"
                            value="{{ old('opening_time', $playground->opening_time) }}"
                    ...
                    <div class="mb-4">
                        ...
                        <input type="time" name="closing_time" id="closing_time"
                            value="{{ old('closing_time', $playground->closing_time) }}"
                ...
                <div class="mb-4">
                    ...
                    <textarea name="description" id="description" rows="3" placeholder="Enter description..."
                        class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 @error('description') border-red-500 @enderror text-sm">{{ old('description', $playground->description) }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-sm mt-1.5">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-2 gap-4 mb-8">
                    <label class="flex items-center cursor-pointer">
                        <input type="checkbox" name="is_active" value="1" {{ $playground->is_active ? 'checked' : '' }}
                ...
                <div class="flex items-center justify-end gap-3 pt-4 border-t border-gray-100">
                    ...
                    <button type="submit"
                        ...
                        <i class="fas fa-save mr-2"></i> Update Playground
                    </button>
                </div>
            </form>
        </div>
