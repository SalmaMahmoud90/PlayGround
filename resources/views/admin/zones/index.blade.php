...
@section('page-subtitle', 'Manage all zones and their cities')
                ...
                <tbody>
                    @forelse($zones as $zone)
                        <tr class="table-row border-t border-gray-50">
                            <td class="py-3 px-4 text-sm text-gray-500">#{{ $zone->id }}</td>
                            <td class="py-3 px-4 font-semibold text-gray-800">{{ $zone->name }}</td>
                            <td class="py-3 px-4 text-sm text-gray-500">{{ $zone->city->name ?? 'N/A' }}</td>
                            <td class="text-center py-3 px-4">
                                <span class="badge {{ $zone->is_active ? 'badge-confirmed' : 'badge-cancelled' }}">
                            ...
                            <td colspan="5" class="text-center py-16">
                                <div class="flex flex-col items-center">
                                    <div class="w-20 h-20 bg-gray-100 rounded-full flex items-center justify-center mb-4">
                                        <i class="fas fa-map-pin text-gray-300 text-3xl"></i>
                                    </div>
                                    <h4 class="text-lg font-semibold text-gray-600">No zones found</h4>
                                    <p class="text-sm text-gray-400 mt-0.5">Get started by creating your first zone</p>
                                    <a href="{{ route('admin.zones.create') }}"
