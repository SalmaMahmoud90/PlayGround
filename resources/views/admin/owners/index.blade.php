...
@section('page-title', 'Owners Management')
@section('page-subtitle', 'Manage all owners and their accounts')
                ...
                <tbody>
                    @forelse($owners as $owner)
                        <tr class="table-row border-t border-gray-50">
                            <td class="py-3 px-4 text-sm text-gray-500">#{{ $owner->id }}</td>
                            <td class="py-3 px-4 font-semibold text-gray-800">{{ $owner->user->name ?? 'N/A' }}</td>
                            <td class="py-3 px-4 text-sm text-gray-600">{{ $owner->company_name }}</td>
                            <td class="py-3 px-4 text-sm text-gray-500">{{ $owner->user->email ?? 'N/A' }}</td>
                            <td class="text-center py-3 px-4">
                                <span class="badge {{ $owner->is_verified ? 'badge-confirmed' : 'badge-pending' }}">
                                    {{ $owner->is_verified ? 'Verified' : 'Pending' }}
                                </span>
                            </td>
                            <td class="text-center py-3 px-4">
                                <span class="badge {{ $owner->user->is_active ? 'badge-confirmed' : 'badge-cancelled' }}">
                        ...
                        <tr>
                            <td colspan="7" class="text-center py-16">
                                <div class="flex flex-col items-center">
                                    ...
                                    <h4 class="text-lg font-semibold text-gray-600">No owners found</h4>
                                    <p class="text-sm text-gray-400 mt-0.5">Get started by creating your first owner</p>
                                    <a href="{{ route('admin.owners.create') }}"
