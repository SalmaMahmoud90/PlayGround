...
@section('page-subtitle', 'Manage all cities and their status')
                ...
                <tbody>
                    @forelse($cities ?? [] as $city)
                        <tr class="table-row border-t border-gray-50">
                            <td class="py-3 px-4 text-sm text-gray-500">#{{ $city->id }}</td>
                            <td class="py-3 px-4 font-semibold text-gray-800">{{ $city->name }}</td>
                            <td class="py-3 px-4 text-sm text-gray-500">{{ $city->zones_count ?? 0 }}</td>
                            <td class="text-center py-3 px-4">
                                <span class="badge {{ $city->is_active ? 'badge-confirmed' : 'badge-cancelled' }}">
                        ...
                        <tr>
                            <td colspan="5" class="text-center py-16">
                                <div class="flex flex-col items-center">
                                    ...
                                    <h4 class="text-lg font-semibold text-gray-600">No cities found</h4>
                                    <p class="text-sm text-gray-400 mt-0.5">Get started by creating your first city</p>
                                    <a href="{{ route('admin.cities.create') }}"
